...
2. 手机读取 Wi-Fi 列表，看到设备附近可用的网络及信号情况（读取时设备也会在后台重新扫描一次）。
...
- 连接：connected/bleMode=true，随后调用 wifi->startScan() 触发一次 Wi-Fi 扫描。
- 断开：两者 false，打印 reason，重新广播。
- Read：仅 WiFiScan 生效；缓存为空返回中文“扫描中”提示，否则返回中文输入提示加网络列表；返回结果后调用 startScan() 触发一次后台扫描。
