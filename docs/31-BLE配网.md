...
> 安全提示：包含 Wi-Fi 密码的原始配网数据和解析结果仅在 `Config::Debug::logBlePayload=true` 时输出，默认关闭。不应在普通运行环境开启。
...
- Write：空值返回；仅在 logBlePayload 开启时逐字符打印完整收到的数据，再对 WiFiConfig 解析。
- 解析：trim，找到第一个 `+`；没有则报错。前段 `toInt()` 为索引，后段 trim 为密码；继续用上次 Read 冻结的 `selectionSSIDs` 解析 configSSID，越界则保存空 SSID并按 logErrors 打印错误。原始 RX 以及 index/pass 解析结果只受 logBlePayload 控制，最后以 release 语义设置 newWiFiConfig。
...
BLE生命周期与通知日志受 logBle 控制；包含密码的 onWrite/解析日志受独立的 logBlePayload 控制且默认关闭。其余重建需保持 UUID 由调用者注入、属性、`索引+密码` 协议、上次 Read 快照解析和一次性消费语义。
