...
当前 `currentAngle` 是软件已经发送的命令角度，不是舵机轴的实测角度。普通三线舵机没有位置反馈，代码无法凭 PWM 信号判断堵转、断电、脱臂或机械卡死，也不能把“软件到达目标”当成“机械实际到位”。

首次运行默认把软件初始角设为 0°并在 attach 后发送 0°命令。程序只是按配置命令角度，无法读取或确认真实轴角；后续可在网页 MANUAL 模式修改初始角并保存到 NVS。
...
- 若需要检测转不动并停止，必须增加反馈硬件：优先选择带位置反馈的舵机/外置编码器或电位器；也可增加电流传感器检测堵转，但必须结合位置、持续时间和机械限位，不能只用一次电流峰值判定。未接反馈前不得宣称具备卡死检测。
...
- 当前目标板的舵机信号引脚为 GPIO6。
- 默认首次运行可用时间推进和角度序列验证：0→100 按 30 递增并在末步钳制到 100，100→0 按 10 递减；NVS 自定义角度则按相同步长规则运行。
