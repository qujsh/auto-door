...
2. 打印三行启动标题。
3. 先调用 `door_.loadRuntimeSettings()` 从 NVS 取得记录的初始角度；没有记录时为 0°。再把该角度作为 `servo_.begin()` 的初始化角并立即输出对应 PWM；这只能命令/假定位置，不能确认真实轴角。其余参数为舵机引脚、更新间隔、开门步长、关门步长。
...
每轮必须依次执行：`wifi_.update()`、`servo_.update()`、`ble_.update()`、`handleWiFiConfig()`。若仍为 Configuring 且 Wi-Fi 已连接，调用 `enterRunningState()`；最后无条件调用 `door_.update()`，保证未配网或断网时本地门控仍可运行。
