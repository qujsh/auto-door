...
flowchart LR
    ...
    Sensor --> Check
    Check -->|"有效"| Convert
    Check -->|"超量程/无目标"| Clear["按最大距离表示前方无人"]
    Check -->|"零值等真正异常"| Invalid["返回 -1"]
...
测距偶尔可能出现无效值，因此模块不会把每次读取都当作可靠结果。零值或尚未初始化等真正异常返回 `-1`，上层会跳过本轮判断；RangeStatus 4 或距离超过配置量程表示当前没有近距离目标，按最大距离返回，使人员离开并重新朝向天空后能够正常推进无人计时和关门。可用数据经过三点中值滤波，减少单次突变造成误判的概率。
...
- 传感器朝向天空时，RangeStatus 4 或超量程读数按最大距离参与标定，因此会建立最大距离基线；目标后来进入有效量程后实时检测，目标离开后又以最大距离表示前方无人。
...
未初始化时返回 -1。创建 `VL53L0X_RangingMeasurementData_t`，调用 `tof.rangingTest(&measurement, false)`。RangeStatus 等于 4 时返回 `maxDistanceMm / 10.0F`，表示量程内没有目标；毫米值为 0 时返回 -1，保留真正异常语义；毫米值超过 maxDistanceMm 时钳制为同一最大厘米距离；其余返回 `RangeMilliMeter / 10.0F` 厘米。
...
- 初始化失败停止应用，这是硬件安全策略。
- 朝向天空的 RangeStatus 4/超量程结果按最大距离视为有效样本，通常可直接完成标定和滤波预热；只有零值等真正异常持续存在时，标定才会在 5 秒后以最大量程兜底，滤波预热也在 5 秒后超时返回。
...
- 毫米值正确转换为厘米。
- 状态 4 和大于 2000mm 返回 200cm，表示量程内无人；零值返回 -1。
