...
按下 GPIO7 上的点动开关也会进入 MANUAL：第一次按下开门，第二次按下关门，之后交替。这样自动测距不会覆盖按钮动作；需要恢复自动检测时，在网页重新选择 AUTO。
...
`toggleFromButton()` 先置 `manualMode=true`；Closed 时设置开门目标并转为 Open，其他状态设置关门目标并转为 Closed；每次都清除 detecting/currentDetect/currentPresent。
