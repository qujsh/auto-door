...
GPIO7 上的点动开关与 AUTO 距离检测并联工作，不会切换到 MANUAL。第一次按下建立“按钮开门请求”，第二次按下取消请求，之后交替。按钮请求或距离判定任意一个成立时都保持开门；因此第二次按下时若门前仍有人，门不会强行关闭，待人员离开后再按正常安全流程关门。网页切回 AUTO 时会清除按钮请求，恢复纯距离控制。
...
首次运行没有 NVS 设置时使用初始角 0°、相对转角 +100°、开门角 100°。这里的“初始角”是软件配置和上电命令，不是从普通三线舵机读取的真实位置；已有网页保存值始终优先。
...
头文件 guard `DOOR_CONTROLLER_H`，包含 Arduino、Preferences、TofSensor、ServoControl、Config。公开接口另含 `toggleFromButton()`、`saveRuntimeSettings(initial,rotation,threshold)` 和 initial/rotation/open/threshold 四个查询。私有：`updateAutoMode(unsigned long)`、`setDoorOpen()`、`setDoorClose()`。

成员在 baseline 后增加 initialAngle、rotationAngle、distanceThresholdCm；其余为 lastSeenTime、closeStartTime、manualMode、buttonOpenRequested、detecting、detectStartTime、currentDistance、currentDiff、currentDetect、currentPresent、lastPrintTime。
...
| 当前 | 条件 | 动作 | 下一状态 |
|---|---|---|---|
| Closed | present 或 buttonOpenRequested | 目标=constrain(initialAngle+rotationAngle,0,180) | Open |
| Open | !present 且 !buttonOpenRequested | closeStartTime=now | WaitingToClose |
| WaitingToClose | present 或 buttonOpenRequested | 无 | Open |
| WaitingToClose | !present、!buttonOpenRequested 且等待达到 closeDelayMs | 目标=initialAngle | Closed |
...
`toggleFromButton()` 反转 `buttonOpenRequested`。变为 true 时立即设置开门目标并转为 Open；变为 false 且 currentPresent=false 时立即设置关门目标并转为 Closed；若仍有人则保持当前开门状态。它不得修改 manualMode 或清除距离检测状态。`setManualMode(false)` 清除 buttonOpenRequested，保证网页选择 AUTO 后恢复纯距离控制。
...
- 等待关门期间重新有人必须恢复 Open。
- 按钮开门请求与人员存在使用逻辑 OR；任一成立均不得进入关门流程。
